...
const APP_URL = "https://alphamarket.co.in";
...
function getClient() {
  ...
  return {
    client: sgMail,
    fromEmail: process.env.SENDGRID_FROM_EMAIL || "hello@alphamarket.co.in",
  };
}
...
export async function sendRegistrationNotification(userData: {
  ...
  try {
    ...
    html += `</table>`;
    if (isAdvisor) {
      html += `<p style="margin-top: 16px; color: #b45309;">This advisor requires admin approval. Please log in to the Admin Panel to review.</p>`;
    }

    await client.send({ to: NOTIFICATION_EMAIL, from: fromEmail, subject, html });
  ...
  try {
    ...
    if (isAdvisor) {
      html += `
        ...
        <p>Your account has been created successfully. Our admin team will review your registration and SEBI credentials.</p>
      ...
      html += `
        ...
        <p>You can now browse investment strategies from SEBI-registered advisors.</p>
      `;
    }
    html += `
    ...
    await client.send({ to: userData.email, from: fromEmail, subject, html });
  ...
  try {
    ...
    const html = `
      <div style="max-width: 700px; margin: 0 auto; font-family: Arial, sans-serif;">
        ...
        <p>This email confirms that you have read and agreed to the platform agreements on <strong>${consentDate}</strong>:</p>
        <div style="margin: 20px 0; padding: 16px; background-color: #f9f9f9; border-left: 4px solid #c53030; border-radius: 4px;">
          <h3 style="margin: 0 0 8px 0;">1. Digital Advisor Participation Agreement & Risk Disclaimer</h3>
          <p style="margin: 8px 0 0 0;"><a href="${APP_URL}/agreements/advisor-participation" style="color: #c53030; font-weight: bold;">View Full Agreement</a></p>
        </div>
        <div style="margin: 20px 0; padding: 16px; background-color: #f9f9f9; border-left: 4px solid #c53030; border-radius: 4px;">
          <h3 style="margin: 0 0 8px 0;">2. Investment Advisor and Research Analyst Services Agreement</h3>
          <p style="margin: 8px 0 0 0;"><a href="${APP_URL}/agreements/ia-ra-services" style="color: #c53030; font-weight: bold;">View Full Agreement</a></p>
        </div>
    ...
    console.log(`Agreement consent email sent to ${userData.email}`);
  } catch (err) {
  ...
  try {
    ...
    const html = `
      <div style="max-width: 700px; margin: 0 auto; font-family: Arial, sans-serif;">
        ...
        <p style="font-size: 14px;">This email confirms that the following agreement has been electronically signed via Aadhaar OTP verification.</p>
        <div style="margin: 20px 0; padding: 16px; background-color: #f9f9f9; border-left: 4px solid #c53030; border-radius: 4px;">
          <h3 style="margin: 0 0 12px 0;">Agreement Details</h3>
          <table style="border-collapse: collapse; width: 100%; max-width: 500px;">
