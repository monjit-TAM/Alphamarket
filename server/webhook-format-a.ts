...
async function buildEquity(event: string, c: any, strategy: any, advisor: any, upstoxStrategyType?: string): Promise<any> {
  ...
  const equityCall: any = {
    ...
    buyDate: (action === "SELL" && !isClosed) ? null : epochMs(c.call_date),
    buyPrice: (action === "SELL" && !isClosed) ? null : bp,
    buyPriceRangeEnd: (action === "SELL" && !isClosed) ? null : toNum(c.buy_range_end),
    buyPriceRangeStart: (action === "SELL" && !isClosed) ? null : (toNum(c.buy_range_start) ?? bp),
    callType: action,
    sellPrice: (action === "SELL" && !isClosed) ? bp : (isClosed ? toNum(c.sell_price) : null),
    sellDate: (action === "SELL" && !isClosed) ? epochMs(c.call_date) : (isClosed ? epochMs(c.exit_date) : null),
...
async function buildFno(event: string, p: any, strategy: any, advisor: any, upstoxStrategyType?: string): Promise<any> {
  ...
  const fnoLeg: any = {
    ...
    buyDate: (action === "SELL" && !isClosed) ? null : epochMs(p.created_at),
    buyPrice: (action === "SELL" && !isClosed) ? null : entryPrice,
    buyPriceRangeEnd: (action === "SELL" && !isClosed) ? null : entryPrice,
    buyPriceRangeStart: (action === "SELL" && !isClosed) ? null : entryPrice,
    callType: action,
