...
// Known ISINs for funds that may be missed by automated parsing
const KNOWN_ISIN_NAMES: Record<string, string> = {
  "INF200K01495": "SBI ELSS Tax Saver Fund - Direct Plan - Growth",
  "INF200K01RA3": "SBI ELSS Tax Saver Fund - Regular Plan - Growth",
  "INF179KA1RW5": "HDFC Small Cap Fund - Direct Plan - Growth",
  "INF179KA1RX3": "HDFC Small Cap Fund - Regular Plan - Growth",
  "INF846K01DP8": "Axis Large Cap Fund - Direct Plan - Growth",
  "INF846K01DQ6": "Axis Large Cap Fund - Regular Plan - Growth",
};
...
export async function parseCASPdf(buffer: Buffer): Promise<CASParseResult> {
  ...
  for (let i = 0; i < holdingsTableLines.length; i++) {
    ...
    // Get clean scheme name from metadata map (preferred) or known ISINs or parse from line
    let schemeName = isinToName.get(isin) || KNOWN_ISIN_NAMES[isin] || "";
    if (!schemeName) {
      ...
      if (units > 0 && nav > 0 && valuation > 0) {
        ...
        if (ratio > 0.8 && ratio < 1.2) {
          ...
          result.holdings.push({
            name: cleanSchemeName(schemeName),
  ...
  // ──────────────────────────────────────────────────────────────────────
  // RESCUE PASS: Find known ISINs that were missed by all strategies
  // ──────────────────────────────────────────────────────────────────────
  const foundISINs = new Set(result.holdings.map(h => h.isin).filter(Boolean));
  const knownISINs = Object.keys(KNOWN_ISIN_NAMES);
  const missingKnown = knownISINs.filter(isin => !foundISINs.has(isin));

  for (const targetISIN of missingKnown) {
    // Search all lines for this ISIN
    for (let i = 0; i < lines.length; i++) {
      if (!lines[i].includes(targetISIN)) continue;
      ...
      while ((m = numRegex.exec(cleaned)) !== null) {
        const v = parseFloat(m[1].replace(/,/g, ""));
        if (!isNaN(v) && v > 0 && v < 100000000) nums.push(v);
      }

      // Try to find units, NAV, valuation where units * NAV ≈ valuation
      let rescued = false;
      for (let a = 0; a < nums.length && !rescued; a++) {
        for (let b = 0; b < nums.length && !rescued; b++) {
          if (a === b) continue;
          const product = nums[a] * nums[b];
          for (let c = 0; c < nums.length; c++) {
            if (c === a || c === b) continue;
            const ratio = product / nums[c];
            if (ratio > 0.85 && ratio < 1.15 && nums[c] > 100) {
              // Found valid triple
              let units = nums[a], nav = nums[b], valuation = nums[c];
              // NAV is typically between 10 and 5000
              if (nav > 5000 || (units < nav && units < 100)) {
                [units, nav] = [nav, units];
              }

              const name = KNOWN_ISIN_NAMES[targetISIN] || "Fund " + targetISIN;
              result.holdings.push({
                name: cleanSchemeName(name),
                isin: targetISIN,
                units,
                nav,
                costValue: valuation, // CAS summary may not always have cost
                currentValue: valuation,
                assetType: "mutual_fund",
              });
              console.log("[CAS Parser] RESCUED: " + targetISIN + " => " + name + " | " + units + " units @ NAV " + nav.toFixed(2) + " = " + valuation);
              rescued = true;
            }
          }
        }
  ...
  console.log("[CAS Parser] Source: " + result.source + ", Total Holdings: " + result.holdings.length + " (rescued: " + (result.holdings.length - foundISINs.size) + " additional)");
  return result;
}
