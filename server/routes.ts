  ...
  app.get("/api/admin/broker-reports/download", requireAdmin, async (req, res) => {
    try {
      ...
      if (format === 'xlsx') {
        ...
        // Sheet 2: Advisor Summary
        const ws2 = workbook.addWorksheet('Advisor Summary');
        const advisorSummary = await db.execute(sql.raw(
          "WITH eq AS (" +
        ...
        ws2.columns = [
          ...
          { header: 'Mapped to Broker', key: 'mapped', width: 16 },
        ];
        for (const row of advisorSummary.rows) { ws2.addRow(row); }
        ws2.getRow(1).font = { bold: true };

        // Sheet 3: Daily Per Advisor
        const ws3 = workbook.addWorksheet('Daily Per Advisor');
        const dailyAdv = await db.execute(sql.raw(
          "WITH eq AS (" +
        ...
        ws3.columns = [
          { header: 'Date', key: 'date', width: 12 },
          { header: 'Advisor', key: 'advisor', width: 30 },
          { header: 'Equity Calls', key: 'equity', width: 12 },
          { header: 'F&O Positions', key: 'fno', width: 14 },
          { header: 'Total', key: 'total', width: 10 },
        ];
        for (const row of dailyAdv.rows) { ws3.addRow(row); }
        ws3.getRow(1).font = { bold: true };

        // Sheet 4: Broker Breakdown
        const ws4 = workbook.addWorksheet('Broker Breakdown');
        const brokerAdv = await db.execute(sql.raw(
        ...
        for (const row of brokerAdv.rows) { ws4.addRow(row); }
