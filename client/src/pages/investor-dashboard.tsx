    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex-1 max-w-6xl mx-auto px-4 md:px-6 py-6 space-y-6 w-full">
        ...
        {telegramStatus?.linked && subscriptions && subscriptions.length > 0 && (
          <div className="flex items-center justify-between gap-2 px-3 py-2 rounded-lg bg-green-50 dark:bg-green-950/20 border border-green-200 dark:border-green-800 text-xs text-green-700 dark:text-green-400">
            <div className="flex items-center gap-2">
              ...
              <span>Telegram alerts active{telegramStatus.username ? ` (@${telegramStatus.username})` : ""} — receiving instant call notifications.</span>
            </div>
            <button onClick={async () => { await apiRequest("POST", "/api/telegram/unlink"); qc.invalidateQueries({ queryKey: ["/api/telegram/status"] }); }} className="text-red-500 hover:text-red-700 font-medium whitespace-nowrap">Disconnect</button>
          </div>
